...
!!! success "The demo method (use every time)"
    **Extract → Prompt → Verify → Save**  

    - **Extract:** Work from one spec/contract section at a time (not the full book).  
    - **Prompt:** Ask for a job-ready output (checklist, table, plan, tracker).  
    - **Verify:** Require section/page references and spot-check 2–3 items in the PDF.  
    - **Save:** Store the verified result where work lives, with a link back to the source PDF and any missing inputs.  

## 1) Extract (small piece only)
...
## 2) Prompt (useful output)

ChatGPT works best when you ask for something concrete and structured.  

Good demo outputs are things you can actually use on a job:
- A checklist.  
- A table you can copy into Excel.  
- A short plan with clear steps and responsibilities.  
- A tracker / log template.  

## 3) Verify (proof + spot-check)
...
Full examples (bad prompts + good prompts + outputs) are kept in our internal [Google Drive:](  
https://drive.google.com/drive/folders/1BzornliazSVh5qxljQ-vz22KFNmRFed9?usp=drive_link)  

??? tip "Quick universal add‑on (copy/paste to force verification)"
    Paste this block at the end of most prompts:  
...
This Prompt Builder converts a rough request into a clear, repeatable prompt ChatGPT can follow.  

### What it does

It turns a plain request into a structured prompt with: **Task → Context → Constraints → Output format → Verification**.  
...
??? example "Concise Prompt Builder (copy/paste)"
    ```text
    READY PROMPT
...
For the full Prompt Builder version (with stricter rules and formatting), use the internal [Google Drive link:]( 
https://drive.google.com/drive/folders/1BzornliazSVh5qxljQ-vz22KFNmRFed9?usp=drive_link)  

!!! danger "Demo safety notes (always follow)"
